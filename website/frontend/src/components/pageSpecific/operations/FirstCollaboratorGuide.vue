<template>
  <section class="guide-panel" aria-labelledby="first-collaborator-heading">
    ...
    <div class="guide-content">
      ...
      <div class="step-indicator">
        <span class="step-badge">
          ...
          {{ t('onboarding.stepOf', { step: 2, total: 4 }) }}
        </span>
      </div>

      <!-- Instructions -->
      <div class="guide-instructions">
        <h3>{{ t('onboarding.collaborator.instructionsTitle') }}</h3>
        <ol class="instructions-list">
          <li>{{ t('onboarding.collaborator.step1') }}</li>
          <li>{{ t('onboarding.collaborator.step2') }}</li>
          <li>{{ t('onboarding.collaborator.step3') }}</li>
        </ol>
      </div>
      ...
      <div v-if="showHelp" class="help-content" aria-label="Additional help">
        <h4>{{ t('onboarding.collaborator.helpTitle') }}</h4>
        <ul class="help-list">
          <li>
            <strong>{{ t('onboarding.collaborator.helpEmail') }}</strong>
            {{ t('onboarding.collaborator.helpEmailDesc') }}
          </li>
          <li>
            <strong>{{ t('onboarding.collaborator.helpPermissions') }}</strong>
            {{ t('onboarding.collaborator.helpPermissionsDesc') }}
          </li>
          <li>
            <strong>{{ t('onboarding.collaborator.helpRole') }}</strong>
            {{ t('onboarding.collaborator.helpRoleDesc') }}
          </li>
          <li>
            <strong>{{ t('onboarding.collaborator.helpNotification') }}</strong>
            {{ t('onboarding.collaborator.helpNotificationDesc') }}
          </li>
        </ul>
      </div>
...
// Computed
const completed = computed(() => collaboratorInvited.value);
...
.guide-icon {
  ...
  border-radius: var(--radius-lg, 1rem);
  background: var(--color-secondary-bg, #ccfbf1);
...
.step-badge {
  ...
  background: var(--color-secondary-bg, #ccfbf1);
  color: var(--color-secondary-dark, #0d9488);
...
.instructions-list li::before {
  ...
  height: 0.5rem;
  border-radius: var(--radius-full, 999px);
  background: var(--color-secondary, #0f766e);
}
